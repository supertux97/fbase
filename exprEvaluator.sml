...
(*Operators to be used before other operators*)
val hiPredOperators = ["*","/"]

(*Finds the value of a given expression tree. Malformed
expressions are handeled.*)
               ...
               in
                 case secondTok of
                   Symbol(Operator(oper)) => 
                        ...
                        if ListUtil.member oper hiPredOperators then 
...
fun evalFromString(txt:string):string =
  Real.toString(evalExprTree(createExprTree(Scanner.trimAndScan(txt),txt),txt))
