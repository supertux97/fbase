...
fun loadRequestedTables(requested:aliasToTablename list) =
  let fun inner( (name,table)::restRequested, map) = 
          let val metadataFile = table ^ metadataFileEnding
              ...
              in
              case size(dataForTable) of
                  0 => raise ErrorHandler.emptyDatafile(table ^ dataFileEnding)
                 |_ => 
                  let
  ...
  handle Util.CouldNotReadFileException => raise ErrorHandler.noSuchTable()
  end
   ...
   case t1 of 
     (t1Row::t1Rows) => 
         ...
         (case StrMap.get(t1Row,#mergeCol mergeInfo) of 
             ...
             |NONE => raise ErrorHandler.invalidMerge("Could not merge tables.Possibly the provided column for merging does not exist in both tables."))
    ...
    case StrMap.get(data,tableName) of
       SOME(s) => 
        ...
        |NONE => raise ErrorHandler.invalidReferer(
          Util.format(
                "The table '$' does not exist. If merging, " ^
...
(*Preforms a complete query by reading the content of the given query and
