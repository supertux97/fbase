...
val syntaxSymbols = [".",",",":","{","}","(", ")","*","|"]
val validSymbols = predicateOperators @ syntaxSymbols
  ...
  in
    if Char.isAlpha(firstChar) then
      let val (alfaToken, rest) = Substring.splitl Char.isAlpha subString
          val token = getTokenByKind(Substring.string(alfaToken) )
      in (token, lineNo) :: scan(Substring.string rest, lineNo)
      end
    else if firstChar = #" " then scan(rmHeadOfString(str), lineNo)
